...
data_stack <- data_stack |>
  ...
  # mutate(Age = Age - (2022 - 2016)) |>  # Age of person in 2016
  # filter(Age >= 50) |>                  # Remove those with were not initially 50+ in 2016
  pivot_and_factorise(time_vary = FALSE) |>
  group_by(ID) |>
  mutate(Age = Age - (2022 - as.numeric(as.character(wave)))) # Make age time varying
...
fit_4a <- nnet::multinom(
  status ~ Gender + Age + Education_tri + Depression + Total_p + status_prev + wave, family = multinomial, 
  data = data_stack, trace = FALSE)
